...
//	GRAFICO
	...
	TF1 *teo = new TF1("teo","3.97028021*((1/(2-cos(x*3.1415/180.)))^2)*( (1/(2-cos(x*3.1415/180.))) + (2-cos(x*3.1415/180.)) - (sin(x*3.1415/180.))^2 )", 0., 70.);


	TCanvas* canvas = new TCanvas("canvas");

	canvas -> cd();
	grafico->SetTitle("Sezione d'urto");
	grafico->GetXaxis()->SetLimits(0., 70.);
	grafico->GetXaxis()->SetTitle("Angolo [#circ]");
	grafico->GetXaxis()->CenterTitle();
	grafico->GetYaxis()->SetTitle("d#sigma/d#Omega 10^{-26} [cm^{2}]");
