...
// file di fondo
  ff.open("finale_notarghet.Spe",fstream::in);
  if (!ff.is_open())  {
   cout << " file non trovato" << endl;
   return 0;
  }
...
// sottrazione del fondo
  ...
  passox=1.;
	...
	y=1.;
  for (int ii=0;ii<Nch;++ii) {
		a = hh->GetBinContent(ii);
		b = hhf->GetBinContent(ii);
		y = a-b;
    if(y<0.) y=yp;
    x+=passox;
    gg->Fill(x,y);
...
// fit con solo gauss per ottenere parametri iniziali
  ...
  cout<<"R(%) = "<<FWHM*100/par[1]<<endl;		
	...
	//dati
	c1->cd();
	hh->SetTitle("60#circ originale");
	hh->SetLineColor(4);
	hh->SetFillColor(4);
	hh->GetXaxis()->SetTitle("Canali");
	hh->GetXaxis()->CenterTitle();
	hh->GetYaxis()->SetTitle("Conteggi");
	hh->GetYaxis()->CenterTitle();
	hh->Draw("hist");
	
	//sovrapposizioni
	c2->cd();
	hhf->SetTitle("60#circ senza targhetta");
  hhf->SetLineColor(4);
	hhf->SetFillColor(4);
