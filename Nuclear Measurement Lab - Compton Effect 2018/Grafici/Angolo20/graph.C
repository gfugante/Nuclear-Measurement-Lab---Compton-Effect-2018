...
// lettura file e creazione istrogramma
  ...
  ifstream ff;
  ff.open("angolo20-23aprile-trovatospento.Spe",fstream::in);
  if (!ff.is_open())  {
  ...
  ff.open("Targhetta_0.Spe",fstream::in);
  if (!ff.is_open())  {
   cout << " file non trovato" << endl;
   return 0;
  }
  ...
  for (int ii=0; ii<Nch; ii++) {
		x+=passox;
		ff >> y;
		gg20->SetPoint(ii,x,y*tempovivo/(100*tempovivo0));
		gg0->Fill(x,y*tempovivo/(1000*tempovivo0));
	}
	 ff.close();

	TGraph *ggf0 = new TGraph(Nch);
	TH1F *ggf = new TH1F("ggf", " spettro finale", Nch, 0.,Nch);
	x=-passox/2.;
...
// fit con solo gauss per ottenere parametri iniziali
  ...
  cout << " area gauss1= " << area_gauss << " +/-" << err_area_gauss << endl;
	...
	c1->cd();
	ggf->SetTitle("Angolo 20#circ");
	ggf->SetLineColor(4);
	ggf->SetFillColor(4);
	ggf->GetXaxis()->SetTitle("Canali");
	ggf->GetXaxis()->CenterTitle();
	ggf->GetYaxis()->SetTitle("Conteggi");
	ggf->GetYaxis()->CenterTitle();
...
//per disegnare le curve parziali
  ...
  pp->Draw("same");
	...
	c3->cd();
	gg0->SetTitle("Angolo 0#circ");
	gg0->SetLineColor(2);
	gg0->SetFillColor(2);
	gg0->GetXaxis()->SetTitle("Canali");
	gg0->GetXaxis()->CenterTitle();
	gg0->GetYaxis()->SetTitle("Conteggi");
	gg0->GetYaxis()->CenterTitle();
	gg0->Draw("samehist");
