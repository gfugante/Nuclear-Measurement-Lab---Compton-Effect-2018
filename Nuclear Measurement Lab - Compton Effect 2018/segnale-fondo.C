...
//    cout << riga << endl;
  ...
  x=-0.5;
// sostituire il valore del numeratore con la posizione del picco del potassio
 //passox=10500./11050.;
 passox=1.;
  for (int ii=0; ii<Nch; ii++) {
     x+=passox;
...
// sottrazione del fondo
  ...
  passox=1.;
  for (int ii=0;ii<Nch;++ii) {
   y= hh ->GetBinContent(ii)- hhf->GetBinContent(ii);
    if(y<=0.) y=yp;
    x+=passox;
    gg->Fill(x,y);
    yp=y;
...
// fit con solo gauss per ottenere parametri iniziali
 ...
 //per disegnare le curve parziali
  ...
  pp->Draw("same");
	...
	cout << endl;
	//cout << "Integral = " << gg->Integral(4550,5250)<< endl;
