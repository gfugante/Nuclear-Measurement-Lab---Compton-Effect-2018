#include <stdio.h>
#include <fstream>
#include <string>
#include <stdlib.h>
#include <math.h>
#include "TROOT.h"
#include "TFile.h"
#include "TH1F.h"
#include "TCanvas.h"
#include "TGraph.h"
#include "TLine.h"
#include "TGraphErrors.h"
#include "TF1.h"
#include "TStyle.h"
#include "TObjArray.h" 
#include "TRandom.h"
#include <stdlib.h>
#include "TPDF.h"
using namespace std;

int main (int argc, char **argv) {

// root -l
// .L gauss_pol.C
// main()
    if(argc!=4){
        cerr<<"Usage:"<<argv[0]<<" <Voltaggio> <InizioPolinomio> <FinePolinomio>"<<endl;
        return -1;
    }
    
	string tipe = "CeBr";
	string cartel = "CeBr3/CeBrhv";
	string num = argv[1];
	string pdf = tipe + num + ".pdf";
	string file = cartel + num + ".Spe"; 


    TPDF save(pdf.c_str());
    gROOT->Reset();
    gStyle->SetOptStat(111111111);
    gStyle->SetOptFit(111111);

    char riga[256];
    float y;
    float passox=1.;
    float  x=-passox/2.;
    Double_t par[12],errpar[12];
    int tempovivo,tempo;
  
	
    int Nch=8000;
    double inip= atof(argv[2]);
    double endp= atof(argv[3]);
    double inig1= inip+20.;
    double endg1= endp-20.;
  

    for (int i=0; i<10; i++){
        par[i]=0;
        errpar[i]=0;
    }

// lettura file e creazione istrogramma

    fstream ff;
    const char *file1 = file.c_str();
    ff.open(file1,fstream::in);
    if (!ff.is_open())  {
        cout << " file non trovato" << endl;
        return 0;
    }

    TH1F *gg = new TH1F("gg", " spettro", Nch, inip-100.,endp+100.);

    for (int ii=0; ii<9; ii++) {
        ff.getline(riga,256);
        cout << riga << endl;
    }
    ff >> tempovivo >> tempo;
    cout << "tempovivo(s)= " << tempovivo << ", tempo(s)= "<< tempo << endl;
    for (int ii=0; ii<3; ii++) {
        ff.getline(riga,256);
        cout << riga << endl;
    }

    for (int ii=0; ii<Nch; ii++) {
        x+=passox;
        ff >> y;
//      if(y<120) //NaI
        if(x>=inip-100. && x<=endp+100.) //CeBr3
            gg->Fill(x,y);
    }
    gg->Write();

    ff.close();


// fit gaus+polinomio 

    TF1 *g1 = new TF1("g1", "gaus", inig1, endg1);
    TF1 *pp = new TF1("pp", "pol1", inip, endp);

    TF1 *total = new TF1("total","gaus(0)+pol1(3)",inip,endp);

// fit con solo gauss per ottenere parametri iniziali
    gg->Fit(g1,"R");
    g1->GetParameters(&par[0]);
    cout << " solo gaus: h=" << par[0] << " mean= " << par[1] << " sig= " << par[2] << endl;
    total->SetParameters(par);
    total->SetLineColor(6);
  
    gg->Fit(total,"R+","",inip,endp);

    total->GetParameters(&par[0]);
    errpar[0]=total->GetParError(0);
    errpar[2]=total->GetParError(2);

    cout << " risultati fit globale" << endl;
    cout << " Chi2= " << total->GetChisquare() << " NDF= "<< total->GetNDF() <<" prob=" <<  total->GetProb() << endl;
    cout << " Chi2rid= " << total->GetChisquare() / total->GetNDF() << endl;
    cout << " gauss parameters:" << " " << par[0] << " " << par[1] << " " << par[2] << endl;
    cout << " errpar[0]="<< errpar[0] << "   errpar[2]="<< errpar[2] <<endl<<endl;
    cout << "------------"<<endl;
    cout << " sig/mean g1= " << par[2]/par[1] << endl;
    cout << " mean g1= " << par[1]<< endl;
    cout << " sigma g1= " << par[2]<< endl;
    cout << "------------"<<endl<<endl;
    float area_gauss=par[0]*par[2]*2.507;
    double arg=pow((par[2]*errpar[0]),2.)+pow((par[0]*errpar[2]),2.);
    float err_area_gauss=2.507*pow(arg,0.5);
    cout << " area gauss1= " << area_gauss << " +/-" << err_area_gauss << endl;

    total->Write("same");

// per disegnare le curve parziali
    g1->SetParameters(&par[0]);
    g1->SetLineColor(5);
    g1->Write("same");
    pp->SetParameters(&par[3]);
    pp->SetLineColor(3);
    pp->Write("same");

return 0;

}
