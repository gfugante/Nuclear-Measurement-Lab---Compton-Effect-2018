...
// y = A + xB
	...
	grafico->SetTitle("");
	grafico->GetXaxis()->SetTitle("Energia [keV]");
	grafico->GetXaxis()->CenterTitle();
	grafico->GetYaxis()->SetRangeUser(0., 8000.);
	grafico->GetYaxis()->SetTitle("Canali");
	grafico->GetYaxis()->CenterTitle();
	grafico->GetYaxis()->SetTitleOffset(1.35);
	fit->GetYaxis()->SetRangeUser(0., 8000.);
	fit->SetLineColor(1);
	fit->SetLineWidth(1);
